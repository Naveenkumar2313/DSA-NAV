...
const Kruskal_EX2 = () => {
  ...
  useLayoutEffect(() => {
    ...
    const sketch = (p) => {
      ...
      const createGraph = (p) => {
        const nodePositions = [
          { value: "A", x: p.width * 0.15, y: p.height * 0.3 },
          { value: "B", x: p.width * 0.35, y: p.height * 0.15 },
          { value: "C", x: p.width * 0.6, y: p.height * 0.2 },
          { value: "D", x: p.width * 0.8, y: p.height * 0.4 },
          { value: "E", x: p.width * 0.5, y: p.height * 0.7 },
          { value: "F", x: p.width * 0.2, y: p.height * 0.8 }
        ];

        nodes = nodePositions.map((pos) => new Node(pos.value, pos.x, pos.y, p));

        edges = [
          new Edge(nodes[0], nodes[1], 2),
          new Edge(nodes[0], nodes[4], 7),
          new Edge(nodes[1], nodes[2], 3),
          new Edge(nodes[1], nodes[4], 8),
          new Edge(nodes[2], nodes[3], 1),
          new Edge(nodes[2], nodes[4], 4),
          new Edge(nodes[3], nodes[4], 6),
          new Edge(nodes[4], nodes[5], 2),
          new Edge(nodes[0], nodes[5], 9),
          new Edge(nodes[3], nodes[5], 5)
        ];
    ...
    <Box sx={styles.container}>
      <Box sx={{ mb: 2, textAlign: "center" }}>
        <Typography variant="h5" gutterBottom sx={{ color: colors.text.primary, fontWeight: 700 }}>
          Kruskal's Algorithm - Complex Graph MST
        </Typography>
      </Box>
