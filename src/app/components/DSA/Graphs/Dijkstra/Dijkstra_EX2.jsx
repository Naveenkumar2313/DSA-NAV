    ...
    const sketch = (p) => {
      ...
      const createGraph = (p) => {
        ...
        const connections = [
          ...
          [0, 3, 2],
          [1, 4, 4],
          [2, 5, 1], // Column connections
          [3, 4, 3],
          [4, 5, 8], // Row 2
          [3, 6, 6],
          [4, 7, 2],
          [5, 8, 3], // Column connections
          [6, 7, 5],
          [7, 8, 4], // Row 3
          [0, 4, 6],
          [2, 4, 5], // Diagonals
        ...
        edges = [];
        connections.forEach(([fromIdx, toIdx, weight]) => {
          edges.push(new Edge(nodes[fromIdx], nodes[toIdx], weight));
          edges.push(new Edge(nodes[toIdx], nodes[fromIdx], weight)); // Undirected
        });
      };
      ...
      const dijkstraStep = () => {
        ...
        if (current.value === DEFAULT_TARGET) {
          ...
          setStatusText(`✓ Target Reached! Total Distance: ${current.distance}`);
          setStepList((prev) => [
            ...prev,
            `Step ${stepNumber++}: Shortest path to '${DEFAULT_TARGET}' found.`
          ]);
          playSound("success");
          searchComplete = true;
          running = false;
        ...
        setStepList((prev) => [
          ...prev,
          `Step ${stepNumber++}: Extract '${current.value}' (dist = ${current.distance})`
        ]);
        playSound("step");
        ...
        for (let edge of neighbors) {
          ...
          if (!neighbor.visited) {
            ...
            if (newDist < neighbor.distance) {
              ...
              setStepList((prev) => [
                ...prev,
                `  -> Relaxed ${neighbor.value}, new dist = ${newDist}`
              ]);
              relaxedAny = true;
              if (!neighbor.inQueue) {
