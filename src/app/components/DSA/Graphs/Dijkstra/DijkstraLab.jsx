...
const DijkstraLab = ({ showSnackbar }) => {
  ...
  const resetSimulation = useCallback(() => {
    ...
    setNodes((nds) => nds.map((n) => ({ ...n, style: {}, data: { ...n.data, distance: "∞" } })));
  ...
  const handleBuildGraph = useCallback(() => {
    ...
    newNodes = Array.from({ length: nodeCount }, (_, i) => ({
      id: `${i}`,
      data: { label: nodeValues[i] || `N${i}`, distance: "∞" },
      position: { x: 0, y: 0 }
    }));
    ...
    if (layoutMode === "manual") {
      ...
      validEdges.forEach((edge) => {
        newEdges.push({
          ...
          type: "smoothstep",
          animated: false,
          data: { weight: parseFloat(edge.weight) },
      ...
      // Auto layout - create a sample weighted graph
      const autoEdges = [];
      if (layoutMode === "circular") {
        for (let i = 0; i < nodeCount; i++) {
          const next = (i + 1) % nodeCount;
          const weight = Math.floor(Math.random() * 10) + 1;
          autoEdges.push({ from: i, to: next, weight });
          
          if (i < nodeCount - 2) {
            const crossWeight = Math.floor(Math.random() * 15) + 1;
            autoEdges.push({ from: i, to: i + 2, weight: crossWeight });
          }
        }
        ...
        for (let i = 0; i < nodeCount; i++) {
          if ((i + 1) % cols !== 0 && i + 1 < nodeCount) {
            const weight = Math.floor(Math.random() * 10) + 1;
            autoEdges.push({ from: i, to: i + 1, weight });
          }
          if (i + cols < nodeCount) {
            const weight = Math.floor(Math.random() * 10) + 1;
            autoEdges.push({ from: i, to: i + cols, weight });
          }
        }
      }
    ...
    setEdges(newEdges);
    setStatusMessage("Graph built. Ready to find shortest path!");
  ...
  function performStep() {
    ...
    if (priorityQueue.length === 0) {
      ...
      if (targetValue && distances[targetValue] !== Infinity) {
        const path = reconstructPath(previous, targetValue);
        setShortestPath(path);
        setStatusMessage(`✓ Shortest path to ${targetValue}: ${path.join(" → ")} (Distance: ${distances[targetValue]})`);
        appendToLog(`\nShortest path found: ${path.join(" → ")}\nTotal distance: ${distances[targetValue]}\n`);
        playSound("success");
      } else {
        setStatusMessage(`Target '${targetValue}' not reachable.`);
        appendToLog(`\nTarget '${targetValue}' not reachable from source.\n`);
        playSound("failure");
      }
      setIsRunning(false);
      setIsFinished(true);
    ...
    newQueue.sort((a, b) => distances[a] - distances[b]);
    const nodeId = newQueue.shift();
    
    const node = nodes.find((n) => n.id === nodeId);
    setCurrentNodeId(nodeId);
    setStatusMessage(`Processing ${node.data.label} (dist: ${distances[nodeId]})`);
    ...
    appendToLog(`Step ${stepCounter}: Visiting ${node.data.label} with distance ${distances[nodeId]}.\n`);
    setStepCounter((c) => c + 1);

    // Check if we reached the target
    if (node.data.label === targetValue) {
      const path = reconstructPath(previous, nodeId);
      setShortestPath(path);
      simulationTimeout.clear();
      playSound("success");
      setStatusMessage(`✓ Found shortest path to ${targetValue}! Distance: ${distances[nodeId]}`);
      appendToLog(`Target found! Path: ${path.join(" → ")}\nTotal distance: ${distances[nodeId]}\n`);
      setIsRunning(false);
      setIsFinished(true);
      return;
    }
    ...
    const neighbors = edges
      .filter((e) => e.source === nodeId && e.data?.weight !== undefined)
      .map((e) => ({ id: e.target, weight: e.data.weight }));
    ...
    neighbors.forEach(({ id: neighborId, weight }) => {
      if (!newVisited.has(neighborId)) {
        ...
        if (newDist < distances[neighborId]) {
          newDistances[neighborId] = newDist;
          newPrevious[neighborId] = nodeId;
          const neighborNode = nodes.find((n) => n.id === neighborId);
          relaxLog += `  Relaxed edge to ${neighborNode.data.label}: ${distances[neighborId]} → ${newDist}\n`;
          
          if (!newQueue.includes(neighborId)) {
  ...
  const reconstructPath = (prev, targetId) => {
    const path = [];
  ...
  }, [isRunning, isFinished, priorityQueue, isStepping]);

  const handleRunPause = () => {
    if (isGraphBuilt && !targetValue.trim()) {
      showSnackbar("Please enter a target node first.", "error");
      return;
    }
    if (nodes.length === 0) {
    ...
    if (isRunning) {
      ...
      if (priorityQueue.length === 0 && visitedSet.size === 0) {
        // Initialize Dijkstra's algorithm
        const initDistances = {};
        nodes.forEach((n) => {
          initDistances[n.id] = n.id === startNodeId ? 0 : Infinity;
        });
        setDistances(initDistances);
        setPrevious({});
        setPriorityQueue([startNodeId]);
        appendToLog(`Starting Dijkstra from ${nodes.find(n => n.id === startNodeId).data.label}\n`);
      }
  ...
  const handlePrevStep = () => {
    ...
    setPriorityQueue(lastState.priorityQueue);
    setDistances(lastState.distances);
    setPrevious(lastState.previous);
    ...
    setShortestPath(lastState.shortestPath);
    setIsRunning(false);
  ...
  useEffect(() => {
    setNodes((nds) =>
      nds.map((node) => {
        ...
        const isInQueue = priorityQueue.includes(node.id);
        const isInPath = shortestPath.includes(node.data.label);
        ...
        if (isInPath && isFinished) {
          ...
          style.backgroundColor = colors.node.inQueue;
          style.color = "white";
        }
        ...
        return {
          ...
          data: {
            ...node.data,
            distance: distance !== undefined ? (distance === Infinity ? "∞" : distance) : "∞"
          }
        };
      })
    );
    ...
    setEdges((eds) =>
      eds.map((edge) => {
        ...
        const isInShortestPath = 
          ...
          shortestPath.some((nodeLabel, i) => {
            if (i === 0) return false;
            const prevNode = nodes.find((n) => n.data.label === shortestPath[i - 1]);
            const currNode = nodes.find((n) => n.data.label === nodeLabel);
            return (
              prevNode &&
              currNode &&
        ...
        return {
          ...
          style: {
            ...edge.style,
            stroke: isInShortestPath
              ? colors.edge.path
              : sourceVisited && targetVisited
              ? colors.edge.visited
              : colors.edge.default,
            strokeWidth: isInShortestPath ? 4 : sourceVisited && targetVisited ? 3 : 2.5,
            opacity: edge.style?.opacity !== undefined ? edge.style.opacity : 1
          },
          labelStyle: {
            fill: colors.text.primary,
            fontWeight: isInShortestPath ? 700 : 500
          }
        };
      })
    );
  }, [currentNodeId, visitedSet, priorityQueue, distances, shortestPath, isFinished, setNodes, setEdges, nodes, colors]);
    ...
    >
      ...
      >
        <Grid container spacing={2} alignItems="center">
          ...
          <Grid item xs={12} md={isGraphBuilt ? 2.5 : 3.5}>
            <Button
              fullWidth
              variant="contained"
              startIcon={<BuildIcon />}
