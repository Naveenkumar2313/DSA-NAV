  ...
  useLayoutEffect(() => {
    ...
    const sketch = (p) => {
      ...
      const createGraph = (p) => {
        const w = p.width;
        const h = p.height;
        const A = new Node("A", p, w * 0.2, h * 0.3);
        const B = new Node("B", p, w * 0.4, h * 0.3);
        const C = new Node("C", p, w * 0.6, h * 0.3);
        const D = new Node("D", p, w * 0.8, h * 0.3);
        const E = new Node("E", p, w * 0.3, h * 0.7);
        const F = new Node("F", p, w * 0.5, h * 0.7);
        const G = new Node("G", p, w * 0.7, h * 0.7);

        A.neighbors.push(B, E);
        B.neighbors.push(A, C, F);
        C.neighbors.push(B, D, G);
        D.neighbors.push(C);
        E.neighbors.push(A, F);
        F.neighbors.push(B, E, G);
        G.neighbors.push(C, F);
      ...
      const drawEdges = (p) => {
        ...
        for (let node of nodes) {
          for (let neighbor of node.neighbors) {
            const edge = [node.value, neighbor.value].sort().join("-");
      ...
      const dfsStep = () => {
        ...
        if (current.visited) {
            if (running && !paused) dfsStep(); // Skip if already visited
            return;
        }
  ...
  return (
    <Box sx={styles.container}>
      ...
      <Box sx={{ p: 1, mb: 2, display: "flex", justifyContent: "center", gap: 1.5 }}>
        <Tooltip title="Reset">
          <IconButton onClick={() => sketchRef.current.reset()} sx={{ color: isDark ? "#ffffff" : colors.text.primary, bgcolor: colors.buttons.resetBg, border: `1px solid ${colors.border}`, "&:hover": { bgcolor: colors.buttons.hoverBg }}}>
            <RestartAltIcon />
          </IconButton>
        </Tooltip>
        <Tooltip title="Previous Step">
          <IconButton onClick={() => sketchRef.current.prevStep()} sx={{ color: colors.text.primary, bgcolor: colors.buttons.bg, border: `1px solid ${colors.border}`, "&:hover": { bgcolor: colors.buttons.hoverBg }}}>
            <ArrowBackIcon />
          </IconButton>
        </Tooltip>
        <Tooltip title="Next Step">
          <IconButton onClick={() => sketchRef.current.step()} sx={{ color: colors.text.primary, bgcolor: colors.buttons.bg, border: `1px solid ${colors.border}`, "&:hover": { bgcolor: colors.buttons.hoverBg }}}>
            <ArrowForwardIcon />
          </IconButton>
        </Tooltip>
        <Tooltip title="Run">
          <IconButton onClick={() => sketchRef.current.run()} sx={{ color: isDark ? "#ffffff" : colors.text.primary, bgcolor: isPlaying ? colors.buttons.pauseBg : colors.buttons.playBg, border: `1px solid ${colors.border}`, "&:hover": { bgcolor: colors.buttons.hoverBg }}}>
            <PlayArrowIcon />
          </IconButton>
        </Tooltip>
        <Tooltip title="Pause">
          <IconButton onClick={() => sketchRef.current.pause()} sx={{ color: isDark ? "#ffffff" : colors.text.primary, bgcolor: colors.buttons.pauseBg, border: `1px solid ${colors.border}`, "&:hover": { bgcolor: colors.buttons.hoverBg }}}>
            <PauseIcon />
          </IconButton>
        </Tooltip>
      </Box>
      ...
      <Box sx={{ mb: 3, display: "flex", justifyContent: "center", width: "100%" }}>
        <Stack direction="row" spacing={2} sx={{ p: 1.5, borderRadius: 2, background: isDark ? "rgba(0,0,0,0.5)" : "rgba(255,255,255,0.7)", flexWrap: "wrap", justifyContent: "center", border: `1px solid ${colors.border}`}}>
      ...
      <Grid container spacing={{ xs: 2, md: 4 }} justifyContent="center" alignItems="flex-start">
        ...
        <Grid item xs={12} md={5} lg={4}>
          <Paper sx={(theme) => ({ p: { xs: 1.5, md: 2 }, width: "100%", display: "flex", flexDirection: "column", gap: theme.spacing(2), height: "100%", mx: "auto", bgcolor: colors.background.paper, backgroundImage: "none", border: `1px solid ${colors.border}`})} elevation={isDark ? 0 : 3}>
            <Box>
              ...
              <Box ref={stepListRef} sx={styles.stepListBox}>
                {stepList.map((step, index) => (
                  <Typography key={index} variant="body2" sx={{ fontFamily: "monospace", whiteSpace: "pre-wrap", mb: 0.5, color: colors.text.primary }}>
                    {step}
                  </Typography>
                ))}
              </Box>
