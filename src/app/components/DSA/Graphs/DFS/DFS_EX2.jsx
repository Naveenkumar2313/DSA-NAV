  ...
  useLayoutEffect(() => {
    ...
    const sketch = (p) => {
      ...
      const createGraph = (p) => {
        const w = p.width;
        const h = p.height;
        const A = new Node("A", p, w * 0.1, h * 0.5);
        const B = new Node("B", p, w * 0.3, h * 0.2);
        const C = new Node("C", p, w * 0.3, h * 0.8);
        const D = new Node("D", p, w * 0.5, h * 0.5);
        const E = new Node("E", p, w * 0.7, h * 0.2);
        const F = new Node("F", p, w * 0.7, h * 0.8);
        const K = new Node("K", p, w * 0.9, h * 0.5);

        A.neighbors.push(B, C);
        B.neighbors.push(A, D, E);
        C.neighbors.push(A, D, F);
        D.neighbors.push(B, C, E, F);
        E.neighbors.push(B, D, K);
        F.neighbors.push(C, D, K);
        K.neighbors.push(E, F);
