...
function buildScenarioSteps(inputStr, label) {
  const steps = [];
  const root = new TrieNode("");
  let totalNodes = 0;

  function snap(msg, sound) {
    steps.push({ trie: cloneTrie(root), msg: `[${label}] ${msg}`, sound });
  }

  snap(`Scenario ${label}: Count unique substrings of "${inputStr}"`, "step");
  ...
  for (let i = 0; i < inputStr.length; i++) {
    ...
    for (let j = 0; j < suffix.length; j++) {
      ...
      if (!node.children[ch]) {
        ...
        snap(`New node '${ch}' → unique substring "${substring}" | Nodes: ${totalNodes}`, "step");
      } else {
    ...
    node.isEnd = true;
    node.state = "inserted";
    snap(`Suffix "${suffix}" inserted ★ | Nodes: ${totalNodes}`, "success");
    resetStates(root);
  ...
  snap(`✅ "${inputStr}": ${totalNodes} nodes = ${totalNodes} unique substrings + empty = ${uniqueCount}`, "success");

  return steps;
}

function buildAllSteps() {
  const stepsA = buildScenarioSteps("abab", "A");
  const stepsB = buildScenarioSteps("aaa", "B");
  ...
  useLayoutEffect(() => {
    ...
    const sketch = (p) => {
      ...
      p.reset = () => {
        ...
        setStatus(
          'Ready — [A] "abab" → 8 unique substrings  |  [B] "aaa" → 4 unique substrings'
        );
        setStepList([]);
        setIsPlaying(false);
        setIsDone(false);
    ...
    <Box sx={sty.container}>
      <Box sx={{ mb: 2, textAlign: "center" }}>
        ...
        <Typography variant="body2" sx={{ color: colors.text.secondary }}>
          <strong>[A]</strong> "abab" → Unique substrings: <strong>8</strong>
          &nbsp;&nbsp;|&nbsp;&nbsp;
          <strong>[B]</strong> "aaa" → Unique substrings: <strong>4</strong>
        </Typography>
      </Box>
