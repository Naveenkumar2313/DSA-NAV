...
function countNodes(node) {
  let c = node.char !== "" ? 1 : 0;
  for (const k of Object.keys(node.children)) c += countNodes(node.children[k]);
  return c;
}

function buildAllSteps(inputStr) {
  const steps = [];
  const root = new TrieNode("");
  let totalNodes = 0;

  function snap(msg, sound) {
    steps.push({ trie: cloneTrie(root), msg, sound, nodeCount: totalNodes });
  }

  snap(`Count unique substrings of "${inputStr}" using a Trie`, "step");
  snap(`Approach: Insert every suffix into a Trie. Each node = one unique substring.`, "step");
  ...
  for (let i = 0; i < inputStr.length; i++) {
    ...
    for (let j = 0; j < suffix.length; j++) {
      ...
      if (!node.children[ch]) {
        ...
        snap(`New node '${ch}' created → new unique substring "${substring}" | Total nodes: ${totalNodes}`, "step");
      } else {
        node.children[ch].state = "highlight";
        const substring = inputStr.slice(i, i + j + 1);
        snap(`Node '${ch}' exists → substring "${substring}" already counted`, "step");
      }
    ...
    resetStates(root);
    snap(`Trie after inserting suffix "${suffix}"`, null);
  }

  // Final highlight — mark all nodes as counted
  function markAllCounted(node) {
    if (node.char !== "") node.state = "counted";
    for (const k of Object.keys(node.children)) markAllCounted(node.children[k]);
  }
  markAllCounted(root);
  const uniqueCount = totalNodes + 1; // +1 for empty substring
  snap(
    `✅ Total Trie nodes = ${totalNodes} (unique non-empty substrings). Including empty substring: ${uniqueCount}`,
    "success"
  );
...
const CUS_EX1 = () => {
  ...
  useLayoutEffect(() => {
    ...
    const sketch = (p) => {
      ...
      p.reset = () => {
        const s = stateRef.current;
        s.steps = buildAllSteps(INPUT_STR);
        s.stepIndex = 0;
        s.running = false;
        s.paused = true;
        displayTrieRef.current = new TrieNode("");
        setStatus(`Ready — String: "${INPUT_STR}" | Unique substrings: 7 (including empty)`);
        setStepList([]);
        setIsPlaying(false);
        setIsDone(false);
      };
    ...
    <Box sx={sty.container}>
      <Box sx={{ mb: 2, textAlign: "center" }}>
        ...
        <Typography variant="body2" sx={{ color: colors.text.secondary }}>
          String: "abc" → Unique substrings (incl. empty): <strong>7</strong> — a, b, c, ab, bc, abc, ""
        </Typography>
      </Box>
