...
function assignPositions(node, W, depth = 0, x0 = 0, x1 = null) {
  ...
  const keys = Object.keys(node.children).sort(),
    tot = countLeaves(node);
  let cur = x0;
  for (const k of keys) {
    const child = node.children[k],
      frac = countLeaves(child) / tot;
...
function buildAllSteps() {
  ...
  function snapA(msg, sound) {
    steps.push({ trie: cloneTrie(rootA), msg: `[A] ${msg}`, sound });
  }
  ...
  while (currA) {
    ...
    snapA(
      `At "${currA.char || "root"}" — end=${currA.isEnd}, children=${
        Object.keys(currA.children).length
      }`,
      "step"
    );
    if (currA.isEnd) {
      currA.state = "branch";
      snapA("End-of-word — stop", "fail");
      break;
    }
    const keysA = Object.keys(currA.children);
    if (!keysA.length) {
      currA.state = "lcp";
      snapA("Leaf — stop", "step");
      break;
    }
    if (keysA.length > 1) {
      currA.state = "branch";
  ...
  function snapB(msg, sound) {
    steps.push({ trie: cloneTrie(rootB), msg: `[B] ${msg}`, sound });
  }
  ...
  rootB.state = "branch";
  snapB(
    `Root has ${keysRoot.length} children: ${keysRoot.sort().join(", ")} — BRANCH immediately!`,
    "fail"
  );
  resetStates(rootB);
  ...
  useLayoutEffect(() => {
    ...
    const sketch = (p) => {
      ...
      p.reset = () => {
        ...
        setStatus(
          'Ready — [A] "interview","interact","interface" → LCP "inter"  |  [B] "dog","racecar","car" → LCP ""'
        );
        setStepList([]);
        setIsPlaying(false);
        setIsDone(false);
  ...
  return (
    <Box sx={sty.container}>
      ...
      <Box sx={{ mb: 2, display: "flex", flexDirection: "column", alignItems: "center" }}>
        ...
        <Typography variant="body1" sx={{ mt: 1.5, color: colors.text.primary }}>
          Status: {status}
        </Typography>
      </Box>
