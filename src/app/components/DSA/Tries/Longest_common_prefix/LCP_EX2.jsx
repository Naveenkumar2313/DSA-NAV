...
// Two scenarios in one step list: first "interview/interact/interface", then "dog/racecar/car"
function buildAllSteps() {
  const steps = [];

  // ── Scenario A: long prefix ──────────────────────────────────────────────
  const wordsA = ["interview", "interact", "interface"];
  const rootA  = new TrieNode("");

  function snapA(msg, sound) { steps.push({ trie: cloneTrie(rootA), msg: `[A] ${msg}`, sound }); }

  snapA(`Scenario A: ${wordsA.map(w => `"${w}"`).join(", ")} → expected LCP "inter"`, "step");

  for (const word of wordsA) {
    snapA(`Inserting "${word}"`, "step");
    let node = rootA;
    for (const ch of word) {
      ...
      snapA(`Follow '${ch}'`, "step");
      node.state = "default";
      node = node.children[ch];
    }
    node.isEnd = true; node.state = "inserted";
    snapA(`"${word}" — end-of-word ★`, "success");
    resetStates(rootA);
    snapA(`Trie after inserting "${word}"`, null);
  }

  snapA("All words inserted. Finding LCP...", "step");
  let currA = rootA, lcpA = "";

  while (currA) {
    currA.state = "highlight";
    snapA(`At "${currA.char || "root"}" — end=${currA.isEnd}, children=${Object.keys(currA.children).length}`, "step");
    if (currA.isEnd) { currA.state = "branch"; snapA("End-of-word — stop", "fail"); break; }
    const keysA = Object.keys(currA.children);
    if (!keysA.length) { currA.state = "lcp"; snapA("Leaf — stop", "step"); break; }
    if (keysA.length > 1) {
      currA.state = "branch";
      snapA(`Branch at "${currA.char || "root"}": ${keysA.sort().join(", ")} — LCP stops`, "fail");
      break;
    }
    const ch = keysA[0];
    currA.state = "lcp"; lcpA += ch;
    snapA(`Single child '${ch}' → LCP = "${lcpA}"`, "step");
    currA.children[ch].state = "highlight";
    currA = currA.children[ch];
  }

  resetStates(rootA);
  let pA = rootA;
  for (const ch of lcpA) { pA.state = "lcp"; pA = pA.children[ch]; pA.state = "lcp"; }
  snapA(`✅ LCP = "${lcpA}"`, "success");

  // ── Scenario B: no common prefix ─────────────────────────────────────────
  const wordsB = ["dog", "racecar", "car"];
  const rootB  = new TrieNode("");

  function snapB(msg, sound) { steps.push({ trie: cloneTrie(rootB), msg: `[B] ${msg}`, sound }); }

  snapB(`Scenario B: ${wordsB.map(w => `"${w}"`).join(", ")} → expected LCP "" (empty)`, "step");

  for (const word of wordsB) {
    snapB(`Inserting "${word}"`, "step");
    let node = rootB;
    for (const ch of word) {
      node.state = "highlight";
      if (!node.children[ch]) { node.children[ch] = new TrieNode(ch); node.children[ch].state = "inserted"; }
      else { node.children[ch].state = "highlight"; }
      snapB(`Follow '${ch}'`, "step");
      node.state = "default";
      node = node.children[ch];
    }
    node.isEnd = true; node.state = "inserted";
    snapB(`"${word}" — end-of-word ★`, "success");
    resetStates(rootB);
    snapB(`Trie after inserting "${word}"`, null);
  }

  snapB("Finding LCP...", "step");
  const keysRoot = Object.keys(rootB.children);
  rootB.state = "branch";
  snapB(`Root has ${keysRoot.length} children: ${keysRoot.sort().join(", ")} — BRANCH immediately!`, "fail");
  resetStates(rootB);
  snapB(`✅ LCP = "" (empty — words share no common prefix)`, "success");
...
const LCP_EX2 = () => {
  ...
  useLayoutEffect(() => {
    ...
    const sketch = (p) => {
      ...
      p.reset = () => {
        const s = stateRef.current;
        s.steps = buildAllSteps();
        s.stepIndex = 0; s.running = false; s.paused = true;
        displayTrieRef.current = new TrieNode("");
        setStatus('Ready — [A] "interview","interact","interface" → LCP "inter"  |  [B] "dog","racecar","car" → LCP ""');
    ...
    <Box sx={sty.container}>
      <Box sx={{ mb: 2, textAlign: "center" }}>
        ...
        <Typography variant="body2" sx={{ color: colors.text.secondary }}>
          <strong>[A]</strong> "interview", "interact", "interface" → LCP: <strong>"inter"</strong>
          &nbsp;&nbsp;|&nbsp;&nbsp;
          <strong>[B]</strong> "dog", "racecar", "car" → LCP: <strong>""</strong> (empty)
        </Typography>
      </Box>
