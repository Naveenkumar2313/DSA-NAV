...
const getSimulationColors = (theme) => {
  ...
  return {
    ...
    background: {
      ...
      default: isDark ? "linear-gradient(135deg,#222A45 0%,#222A45 100%)"
    ...
    stepListBox: {
      background: isDark ? "linear-gradient(145deg,#222A45,#2d3548)"
...
function buildLCPSteps(words) {
  ...
  while (curr) {
    ...
    if (curr.isEnd) {
      curr.state = "branch";
      snap(`Node "${curr.char}" is end-of-word — a word ends here, LCP cannot extend further`, "fail");
      break;
    }
    ...
    if (childKeys.length > 1) {
      curr.state = "branch";
      snap(
        `Node "${curr.char || "root"}" has ${childKeys.length} children (${childKeys.sort().join(", ")}) — this is a BRANCH POINT. LCP ends here`,
        "fail"
      );
      break;
    }
...
const LCPLab = ({ showSnackbar }) => {
  ...
  const handleStep = useCallback(() => {
    if (!stepsRef.current.length) { showSnackbar("Add words and click Find LCP first.", "warning"); return; }
    if (stepIndexRef.current >= stepsRef.current.length) { showSnackbar("Already complete.", "info"); return; }
    applyStep(stepIndexRef.current++);
  }, [applyStep, showSnackbar]);

  const handleRun = useCallback(() => {
    if (isPlaying) { clearTimeout(timeoutRef.current); setIsPlaying(false); return; }
    if (!stepsRef.current.length) { showSnackbar("Add words and click Find LCP first.", "warning"); return; }
    if (stepIndexRef.current >= stepsRef.current.length) { showSnackbar("Already complete.", "info"); return; }
    setIsPlaying(true);
    const run = () => {
      if (stepIndexRef.current >= stepsRef.current.length) { setIsPlaying(false); return; }
      applyStep(stepIndexRef.current++);
  ...
  const handleAddWord = useCallback(() => {
    const w = wordInput.trim().toLowerCase().replace(/[^a-z]/g, "");
    if (!w) { showSnackbar("Enter a valid word (a-z).", "warning"); return; }
    if (wordList.includes(w)) { showSnackbar(`"${w}" already added.`, "info"); return; }
    if (wordList.length >= 8) { showSnackbar("Maximum 8 words.", "warning"); return; }
  ...
  const handleFindLCP = useCallback(() => {
    ...
    setIsPlaying(false);
    if (wordList.length < 2) { showSnackbar("Add at least 2 words.", "warning"); return; }
  ...
  const inputSx = {
    "& .MuiOutlinedInput-root": {
      color: colors.input.text, bgcolor: colors.input.background,
  ...
  return (
    <Paper sx={{ p: { xs: 2, sm: 3 }, background: colors.background.default,
                 borderRadius: "20px", border: "1px solid #e0e0e0", boxShadow: colors.paper.boxShadow }}>
      ...
      <Paper sx={{ p: 2, mb: 2, bgcolor: colors.background.paper, border: "1px solid #e0e0e0" }}>
        <Stack direction={{ xs: "column", sm: "row" }} spacing={2} alignItems="center">
          <TextField
            size="small" label="Add word (a-z)" value={wordInput}
            onChange={e => setWordInput(e.target.value.toLowerCase().replace(/[^a-z]/g, ""))}
            onKeyPress={e => e.key === "Enter" && handleAddWord()}
            sx={{ ...inputSx, flexGrow: 1 }}
          />
          <Button variant="outlined" startIcon={<AddIcon />} onClick={handleAddWord}
            sx={{ borderColor: colors.input.focusBorder, color: colors.input.focusBorder,
                  "&:hover": { borderColor: colors.input.focusBorder, bgcolor: alpha(colors.input.focusBorder, 0.08) } }}>
            Add
          </Button>
          <Button variant="contained" onClick={handleFindLCP} disabled={wordList.length < 2}
            sx={{ bgcolor: colors.nodeStroke.lcp,
          ...
          <Stack direction="row" spacing={1} flexWrap="wrap" sx={{ mt: 2, gap: 1 }}>
            {wordList.map(w => (
              <Chip
                ...
                sx={{ bgcolor: colors.nodeFill.inserted, color: colors.text.primary,
      ...
      {/* LCP result banner */}
      {lcpResult !== null && (
        <Box sx={{ mb: 2, p: 1.5, borderRadius: 2, textAlign: "center",
                   bgcolor: lcpResult.length ? colors.nodeFill.lcp : colors.nodeFill.branch,
                   border: `1.5px solid ${lcpResult.length ? colors.nodeStroke.lcp : colors.nodeStroke.branch}` }}>
      ...
      <Box sx={{ mb: 2, display: "flex", flexDirection: "column", alignItems: "center" }}>
        <Stack direction="row" spacing={1.5} flexWrap="wrap" justifyContent="center"
          ...
          <LegendItem color={colors.nodeFill.default}   stroke={colors.nodeStroke.default}   text="Default"       textColor={colors.text.primary} />
        </Stack>
        <Typography variant="body1" sx={{ mt: 1.5, color: colors.text.primary, textAlign: "center" }}>
          {status}
        </Typography>
      </Box>
      ...
      <Grid container spacing={3}>
        <Grid item xs={12} lg={8}>
          <Box ref={canvasRef} sx={{ position: "relative", borderRadius: "24px", overflow: "hidden",
               boxShadow: colors.paper.boxShadow, background: colors.background.canvas,
               border: "1px solid #e0e0e0", minHeight: "480px", height: "100%" }} />
        </Grid>
        <Grid item xs={12} lg={4}>
          <Paper sx={{ p: 2, minHeight: "480px", maxHeight: "480px", display: "flex",
                       flexDirection: "column", bgcolor: colors.background.paper, border: "1px solid #e0e0e0" }}>
            <Stack direction="row" justifyContent="space-between" alignItems="center">
              ...
              <Stack direction="row" spacing={0.5}>
                ...
                <Tooltip title="Next Step">
                  <IconButton size="small" onClick={handleStep} disabled={isPlaying}
                    sx={{ color: colors.text.primary, background: ic.background, border: `1px solid ${ic.border}`, "&:hover": { background: ic.hoverBackground } }}>
                    <ArrowForwardIcon />
                  </IconButton>
                </Tooltip>
                <Tooltip title={isPlaying ? "Pause" : "Run"}>
                  <IconButton size="small" onClick={handleRun}
                    sx={{ background: isPlaying ? colors.nodeFill.highlight : colors.nodeFill.found }}>
                    {isPlaying ? <PauseIcon /> : <PlayArrowIcon />}
                  </IconButton>
                </Tooltip>
              </Stack>
            </Stack>
            <Box ref={stepListRef} sx={{
              ...
              {stepList.map((s, i) => (
                <Typography key={i} variant="body2" sx={{ fontFamily: "monospace", whiteSpace: "pre-wrap", mb: 0.5, color: colors.text.primary }}>
                  {s}
                </Typography>
              ))}
            </Box>
...
const LegendItem = ({ color, stroke, text, textColor }) => (
  <Stack direction="row" spacing={0.8} alignItems="center">
    <Box sx={{ width: 18, height: 18, borderRadius: "50%", backgroundColor: color, border: `2px solid ${stroke}` }} />
