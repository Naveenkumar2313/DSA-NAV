...
const getSimulationColors = (theme) => {
  ...
  return {
    ...
    node: {
      ...
      cutoff: isDark ? "#2a2a2a" : "#9E9E9E",
      cutoffborder: isDark ? "#333333" : "#616161"
    },
    edge: {
...
const DLSLab = ({ showSnackbar }) => {
  ...
  function performStep() {
    ...
    if (simulationStack.length === 0) {
      ...
      if (wasFound) {
        ...
        if (targetExists && targetExists.data.depth > depthLimit) {
          finalMessage = ` Cutoff Failure: '${targetValue}' exists but is beyond depth limit ${depthLimit}.`;
        } else {
    ...
    if (node.data.depth > depthLimit) {
      ...
      appendToLog(
        `Step ${stepCounter}: Node ${node.data.label} (depth ${node.data.depth}) exceeds limit ${depthLimit}. Cutoff.\n`
      );
    ...
    setNodes((nds) =>
      nds.map((node) => {
        ...
        if (isFound) {
          ...
          style.backgroundColor = colors.node.cutoff;
          style.color = "white";
          style.border = `3px solid ${colors.node.cutoffBorder}`;
  ...
  }, [currentNodeId, visitedSet, setNodes, setEdges, targetValue, depthLimit, isFinished, colors]);
          ...
          {layoutMode === "manual" && (
            <Grid item xs={12}>
              ...
              >
                ...
                >
                  Edges (Parent Index → Child Index)
                </Typography>
