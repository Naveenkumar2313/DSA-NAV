...
const getSimulationColors = (theme) => {
  ...
  return {
    ...
    pathFound: {
      main: isDark ? "#4ade80" : "#2ecc71",
      light: isDark ? "#86efac" : "#a3e4d7"
    },
    pathNotFound: {
      main: isDark ? "#f87171" : "#e74c3c",
      light: isDark ? "#fca5a5" : "#f5b7b1"
    },
    background: {
    ...
    node: {
      ...
      processed: isDark ? "#4ade80" : "#1976D2",
      processedBorder: isDark ? "#22c55e" : "#1976D2",
      pathFound: isDark ? "#4ade80" : "#2ecc71",
      pathFoundBorder: isDark ? "#22c55e" : "#27ae60"
    },
    edge: {
...
const PSLab = ({ showSnackbar }) => {
  ...
  const [pathFoundNodes, setPathFoundNodes] = useState(new Set());
  const [pathFoundEdges, setPathFoundEdges] = useState(new Set());

  // Algorithm-specific state
  const [dfsSteps, setDfsSteps] = useState([]);
  const [currentStepIndex, setCurrentStepIndex] = useState(-1);
  const [runningSums, setRunningSums] = useState({});
  const [foundPath, setFoundPath] = useState(false);
  const [successPathNodeIds, setSuccessPathNodeIds] = useState([]);
  ...
  // --- Pre-compute DFS steps for path sum algorithm ---
  const precomputePathSumSteps = useCallback((adjacency, rootId, nodeMap, target) => {
    const steps = [];
    const childrenMap = {};
    let pathFound = false;
    let successPath = [];
    ...
    const dfs = (nodeId, currentSum, path) => {
      const nodeLabel = nodeMap[nodeId];
      const nodeVal = parseInt(nodeLabel) || 0;
      ...
      steps.push({
        ...
        nodeValue: nodeLabel,
        runningSum: newSum,
        message: `Visiting node '${nodeLabel}', running sum = ${newSum}`
      });
      ...
      if (children.length === 0) {
        ...
        steps.push({
          ...
          path: [...path, nodeId],
          message: isMatch
            ? `🎯 Leaf '${nodeLabel}': sum = ${newSum} === ${target}. PATH FOUND!`
            : `❌ Leaf '${nodeLabel}': sum = ${newSum} ≠ ${target}. Backtrack.`
        });
        if (isMatch && !pathFound) {
          pathFound = true;
          successPath = [...path, nodeId];
        }
        return;
      }

      for (let i = 0; i < children.length; i++) {
        dfs(children[i], newSum, [...path, nodeId]);
        ...
        steps.push({
          ...
          nodeValue: nodeLabel,
          runningSum: newSum,
          message: i < children.length - 1
            ? `Back to '${nodeLabel}' (sum = ${newSum}), try next child`
            : `Back to '${nodeLabel}' (sum = ${newSum}), done with children`
        });
      }
    };

    dfs(rootId, 0, []);
    return { steps, pathFound, successPath };
  }, []);
  ...
  const resetSimulation = useCallback(() => {
    ...
    setPathFoundNodes(new Set());
    setPathFoundEdges(new Set());
    setTraversalLog("");
    setStepCounter(1);
  ...
  const handleBuildTree = useCallback(() => {
    ...
    // Pre-compute path sum steps
    const nodeMap = {};
    newNodes.forEach((n) => { nodeMap[n.id] = n.data.label; });
    const { steps, pathFound: pf, successPath: sp } = precomputePathSumSteps(adjacency, rootNodeId, nodeMap, targetSum);
    setDfsSteps(steps);
    setFoundPath(pf);
    setSuccessPathNodeIds(sp);

    setStatusMessage(`Tree built. Target Sum = ${targetSum}. Ready!`);
    showSnackbar("Tree built successfully!", "success");
    setIsTreeBuilt(true);
  }, [nodeCount, nodeValues, edgesInput, layoutMode, targetSum, resetSimulation, showSnackbar, precomputePathSumSteps]);
  ...
  function performStep() {
    ...
    if (currentStepIndex >= dfsSteps.length - 1) {
      // All steps complete
      if (foundPath) {
        setStatusMessage(`✅ Algorithm complete! Path found with sum = ${targetSum}`);
        appendToLog(`\n✅ Algorithm complete! Path found with sum = ${targetSum}\n`);
        playSound("success");
        highlightSuccessPath();
      } else {
        setStatusMessage(`❌ Algorithm complete! No path found with sum = ${targetSum}`);
        appendToLog(`\n❌ Algorithm complete! No path found with sum = ${targetSum}\n`);
        playSound("failure");
      }
      setIsRunning(false);
      setIsFinished(true);
    ...
    if (step.type === "visit") {
      ...
      setRunningSums((prev) => ({ ...prev, [step.nodeId]: step.runningSum }));
      setStatusMessage(`Visiting: ${step.nodeValue}, sum = ${step.runningSum}`);
  ...
  const highlightSuccessPath = () => {
    if (successPathNodeIds.length === 0) return;

    const pathNodes = new Set(successPathNodeIds);
    const pathEdgeKeys = new Set();

    for (let i = 0; i < successPathNodeIds.length - 1; i++) {
      pathEdgeKeys.add(`${successPathNodeIds[i]}->${successPathNodeIds[i + 1]}`);
    }

    setPathFoundNodes(pathNodes);
    setPathFoundEdges(pathEdgeKeys);
  };
  ...
  const handlePrevStep = () => {
    ...
    setPathFoundNodes(lastState.pathFoundNodes);
    setPathFoundEdges(lastState.pathFoundEdges);
    ...
    setRunningSums(lastState.runningSums);
    setIsRunning(false);
  ...
  useEffect(() => {
    setNodes((nds) =>
      nds.map((node) => {
        ...
        const isOnFoundPath = pathFoundNodes.has(node.id);
        const sum = runningSums[node.id];
        ...
        if (isOnFoundPath) {
          style.backgroundColor = colors.node.pathFound;
          style.color = "white";
          style.border = `3px solid ${colors.node.pathFoundBorder}`;
          style.boxShadow = `0 0 15px ${colors.node.pathFound}`;
        } else if (isCurrent) {
    ...
    setEdges((eds) =>
      eds.map((edge) => {
        ...
        const isOnPath = pathFoundEdges.has(edgeKey);
  ...
  }, [currentNodeId, visitedSet, processedSet, pathFoundNodes, pathFoundEdges, runningSums, setNodes, setEdges, colors]);
  ...
  const handleNodeCountChange = (event) => {
    ...
    setNodeCount(count);
    setNodeValues(Array.from({ length: count }, (_, i) => String(i + 1)));
    setEdgesInput(
    ...
    >
      ...
      {isTreeBuilt && (
        <Typography
          variant="h6"
          align="center"
          sx={{ mb: 2, color: foundPath ? colors.pathFound.main : colors.pathNotFound.main, fontWeight: 600 }}
        >
      ...
      >
        <Grid container spacing={2} alignItems="center">
          ...
          {/* Node value inputs */}
          {isTreeBuilt === false && nodeCount > 0 && (
            <Grid item xs={12}>
              <Box sx={{ p: 1.5, border: `1px solid ${colors.border}`, borderRadius: 2 }}>
                  ...
                  {Array.from({ length: nodeCount }).map((_, i) => (
                    <Grid item xs={3} sm={2} md={1.5} key={i}>
                      <TextField
                        ...
                        value={nodeValues[i] || ""}
                        onChange={(e) => {
                          const newVals = [...nodeValues];
                          newVals[i] = e.target.value;
                          setNodeValues(newVals);
                        }}
                        sx={{
                          "& .MuiOutlinedInput-root": {
                            color: colors.text.primary,
