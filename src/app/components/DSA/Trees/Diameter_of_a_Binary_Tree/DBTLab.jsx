...
const getSimulationColors = (theme) => {
  ...
  return {
    ...
    node: {
      ...
      diameterPath: isDark ? "#ff6b6b" : "#e74c3c",
      diameterPathBorder: isDark ? "#ef4444" : "#c62828"
    },
    edge: {
...
const DBTLab = ({ showSnackbar }) => {
  ...
  const [diameterPathNodes, setDiameterPathNodes] = useState(new Set());
  const [diameterPathEdges, setDiameterPathEdges] = useState(new Set());
  ...
  // --- Pre-compute DFS steps for diameter algorithm ---
  const precomputeDiameterSteps = useCallback((adjacency, rootId, nodeMap) => {
    const steps = [];
    const childrenMap = {};
    ...
    // Store children map in a ref so highlightDiameterPath can use it later
    childrenMapRef.current = childrenMap;

    // DFS post-order to compute height and diameter
    const dfs = (nodeId) => {
      ...
      if (children.length >= 1) {
        ...
        steps.push({
          ...
          height: leftHeight,
          message: `Back to '${nodeLabel}': left height = ${leftHeight}`
        });
      }
      ...
      if (children.length >= 2) {
        ...
        steps.push({
          ...
          height: rightHeight,
          message: `Back to '${nodeLabel}': right height = ${rightHeight}`
        });
      }
      ...
      steps.push({
        ...
        localDiameter,
        nodeHeight,
        message: `Node '${nodeLabel}': leftH=${leftHeight}, rightH=${rightHeight}, diameter through node=${localDiameter}, height=${nodeHeight}`
      });
  ...
  const resetSimulation = useCallback(() => {
    ...
    setDiameterPathNodes(new Set());
    setDiameterPathEdges(new Set());
    setTraversalLog("");
    setStepCounter(1);
  ...
  const handleBuildTree = useCallback(() => {
    ...
    // Store original labels in a ref so they are never mutated
    const origLabels = {};
    ...
    // Pre-compute diameter steps
    const nodeMap = {};
    newNodes.forEach((n) => { nodeMap[n.id] = n.data.label; });
    const steps = precomputeDiameterSteps(adjacency, rootNodeId, nodeMap);
    setDfsSteps(steps);

    setStatusMessage("Tree built. Ready to compute diameter!");
    showSnackbar("Tree built successfully!", "success");
    setIsTreeBuilt(true);
  }, [nodeCount, nodeValues, edgesInput, layoutMode, resetSimulation, showSnackbar, precomputeDiameterSteps]);
  ...
  function performStep() {
    ...
    if (currentStepIndex >= dfsSteps.length - 1) {
      // All steps complete — compute final diameter
      setStatusMessage(`Algorithm complete! Diameter = ${currentDiameter}`);
      appendToLog(`\nAlgorithm complete! Diameter = ${currentDiameter}\n`);
      playSound("success");
      setIsRunning(false);
      setIsFinished(true);
      setCurrentNodeId(null);
    ...
    if (step.type === "visit") {
      ...
      if (step.localDiameter > currentDiameter) {
        setCurrentDiameter(step.localDiameter);
      }
  ...
  const highlightDiameterPath = () => {
    // Find the node that contributes to max diameter
    const computeSteps = dfsSteps.filter((s) => s.type === "compute");
    let maxDiam = 0;
    let maxNodeId = null;

    for (const s of computeSteps) {
      if (s.localDiameter >= maxDiam) {
        maxDiam = s.localDiameter;
        maxNodeId = s.nodeId;
      }
    }
    ...
    const pathNodes = new Set([maxNodeId]);
    const pathEdgeKeys = new Set();
    const cMap = childrenMapRef.current;

    // Trace the tallest path down a subtree from a given node
    const traceDown = (nodeId) => {
      const children = cMap[nodeId] || [];
      if (children.length === 0) return;

      // Pick the child with the greatest height
      let bestChild = null;
      let bestHeight = -1;
      for (const childId of children) {
        const ch = nodeHeights[childId];
        if (ch !== undefined && ch > bestHeight) {
          bestHeight = ch;
          bestChild = childId;
        }
      }
      ...
      if (bestChild !== null) {
        pathNodes.add(bestChild);
        pathEdgeKeys.add(`${nodeId}->${bestChild}`);
        traceDown(bestChild);
      }
    };

    // Trace left subtree (first child)
    const children = cMap[maxNodeId] || [];
    if (children.length >= 1) {
      pathNodes.add(children[0]);
      pathEdgeKeys.add(`${maxNodeId}->${children[0]}`);
      traceDown(children[0]);
    }
    // Trace right subtree (second child)
    if (children.length >= 2) {
      pathNodes.add(children[1]);
      pathEdgeKeys.add(`${maxNodeId}->${children[1]}`);
      traceDown(children[1]);
    }

    setDiameterPathNodes(pathNodes);
    setDiameterPathEdges(pathEdgeKeys);
  };
  ...
  const handlePrevStep = () => {
    ...
    setDiameterPathNodes(lastState.diameterPathNodes);
    setDiameterPathEdges(lastState.diameterPathEdges);
    ...
    setCurrentDiameter(lastState.currentDiameter);
  ...
  useEffect(() => {
    setNodes((nds) =>
      nds.map((node) => {
        ...
        const isOnDiameterPath = diameterPathNodes.has(node.id);
        ...
        if (isOnDiameterPath) {
          style.backgroundColor = colors.node.diameterPath;
          style.color = "white";
          style.border = `3px solid ${colors.node.diameterPathBorder}`;
          style.boxShadow = `0 0 15px ${colors.node.diameterPath}`;
        } else if (isCurrent) {
        ...
        // Use the ORIGINAL label from the ref, never the already-modified node.data.label
    ...
    setEdges((eds) =>
      eds.map((edge) => {
        ...
        const isOnDiamPath = diameterPathEdges.has(edgeKey);
        ...
        if (isOnDiamPath) {
          return {
            ...
            animated: true,
            style: {
              stroke: colors.edge.diameterPath,
              strokeWidth: 4
            }
          };
        }
  ...
  }, [currentNodeId, visitedSet, processedSet, diameterPathNodes, diameterPathEdges, nodeHeights, setNodes, setEdges, colors]);
  ...
  return (
    ...
    >
      ...
      >
        Diameter of Binary Tree — Simulator
      </Typography>
      ...
      {isTreeBuilt && (
        <Typography
          variant="h6"
          align="center"
          sx={{ mb: 2, color: colors.diameter.main, fontWeight: 600 }}
        >
          Current Diameter: {currentDiameter}
        </Typography>
      )}
