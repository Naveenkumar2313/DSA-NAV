...
const DBT_EX1 = () => {
  ...
  useLayoutEffect(() => {
    ...
    // Pre-compute all DFS steps for the diameter algorithm
    function precomputeSteps(node, allNodes) {
      if (!node) return 0;
      ...
      const leftHeight = precomputeSteps(node.left, allNodes);
      ...
      const rightHeight = precomputeSteps(node.right, allNodes);
    ...
    const sketch = (p) => {
      ...
      const createTree = (p) => {
        ...
        const w = p.width;
        const h = p.height;
        layoutTree(A, w / 2, h * 0.13, w * 0.22, h * 0.22);
      ...
      const drawEdges = (p) => {
        ...
        // Draw diameter path edges with highlight
      ...
      const drawDiameterInfo = (p) => {
        // Draw current diameter value in top-right corner
      ...
      const executeStep = () => {
        if (currentStepIndex >= dfsSteps.length - 1) {
          // All steps done — find and highlight the diameter path
      ...
      const findAndHighlightDiameterPath = () => {
        // Find the node that gives max diameter and trace the path
        let maxDiam = 0;
        let maxNode = null;
        ...
        if (maxNode) {
          // Trace left path down
          const tracePath = (node, direction) => {
            if (!node) return;
            node.isOnDiameterPath = true;
            const child = direction === "left" ? node.left : node.right;
            if (child) {
              // Follow the taller child
              const lh = child.left && child.left.heightValue !== null ? child.left.heightValue : 0;
              const rh = child.right && child.right.heightValue !== null ? child.right.heightValue : 0;
              diameterEdges.push({ from: node, to: child });
              child.isOnDiameterPath = true;
              if (child.heightValue > 1) {
                if (lh >= rh && child.left) {
                  tracePath(child, "left");
                } else if (child.right) {
                  tracePath(child, "right");
                }
              }
            }
          };
    ...
    <Box sx={styles.container}>
      <Box sx={{ mb: 2, textAlign: "center" }}>
        <Typography variant="h5" gutterBottom sx={{ color: colors.text.primary, fontWeight: 700 }}>
          Example 1: Finding Diameter of a Binary Tree
        </Typography>
